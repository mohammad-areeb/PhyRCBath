...
class Perception:
    ...
    def __init__(self, env:BathingEnv): 
        ...
        self.sky_cam = env.InstanceObject(name="Camera", id=123456, attr_type=attr.CameraAttr)
        self.manikin_cam = env.InstanceObject(name="Camera", id=223456, attr_type=attr.CameraAttr)
        #self.camera = self.environment.get_camera()
        self.sky_cam.SetTransform(position=[0, 3.8, 1.0], rotation=[90, 0, 0])
        self.manikin_cam.SetTransform(position=[0, 3.8, 0], rotation=[90, 0, 0])
    ...
    def manikincam_to_world(self, camera_x, camera_z) -> list: 
        """
        Get the global coordinates in the world from the (x, y) pixel coordinate in the manikin cam image
        :param camera_x: the x-coordinate pixel (horizontal)
        :param camera_z: the y-coordinate pixel (vertical)
        :return: a list of the (x, y, z) world coordinates of the pixel coordinate (where y = 0.0 as height data cannot be inferred)
        """

        world_x = (((camera_x - self.camera_min) * 4) / (self.camera_range - 1)) + (-2)
        world_z = ((((self.img_size - camera_z) - self.camera_min) * 4) / (self.camera_range - 1)) + -2
        return([world_x, 0.0, world_z])
